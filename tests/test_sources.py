...
class TestAlaa(unittest.TestCase):
    ...
    def test_reads_id_state_city_and_contacts(self):
        record = self.by_name["EverHeart Labradoodles"]
        self.assertEqual(record.registry_id, "9292")
        self.assertEqual((record.city, record.region), ("Chagrin Falls", "OH"))
        self.assertEqual(record.domain, "everheartdoodles.com")
        self.assertEqual(record.email, "hello@everheartdoodles.com")
        self.assertEqual(record.phone, "440-555-0188")

    def test_kennel_name_cannot_manufacture_an_award(self):
        # "Silver Creek" holds a Platinum paw and no Silver paw.
        awards = self.by_name["Silver Creek Australian Labradoodles"].awards
        self.assertEqual(awards, ["Platinum (image)"])

    def test_word_boundary_stops_golden_matching_gold(self):
        awards = self.by_name["Golden Prairie Doodles"].awards
        self.assertEqual(awards, ["Gold (image)"])

    def test_text_only_award_is_marked_as_weaker_evidence(self):
        self.assertEqual(self.by_name["Draycot Meadows"].awards,
                         ["Platinum (text_paw)"])

    def test_no_award_means_no_award(self):
        self.assertEqual(self.by_name["Ridgeline Labradoodles"].awards, [])

    def test_bare_text_mention_is_flagged_not_trusted(self):
        block = parse("<tr><td>Platinum Ridge Kennels</td><td>silver</td></tr>")
        awards = dict(detect_awards(block, "Platinum Ridge Kennels"))
...
class TestWala(unittest.TestCase):
    ...
    def test_table_layout(self):
        ...
        first = records[0]
        self.assertEqual(first.registry_id, "WALA-0619-00755")
        self.assertEqual(first.kennel, "Hastings Hollow Labradoodles")
        self.assertEqual(first.owner, "Monique Algera")
        self.assertEqual((first.city, first.region), ("Hastings", "MN"))
    ...
    def test_repeater_layout_and_per_record_tier(self):
        ...
        self.assertEqual(tiers["Hastings Hollow Labradoodles"], ["Three Diamond"])
        self.assertEqual(tiers["Cascade Summit Australian Labradoodles"],
                         ["Two Diamond"])
    ...
    def test_merge_keeps_strongest_rating_first(self):
        ...
        self.assertEqual(merged["WALA-0619-00755"][0], "Three Diamond")
